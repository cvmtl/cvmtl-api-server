...
function point(latlon) {
    var sequelize = this.sequelize;
    return sequelize.fn('ST_GeomFromText', `POINT(${latlon[0]} ${latlon[1]})`);
}

function polygon(polygon) {
    var sequelize = this.sequelize;
    return sequelize.fn('ST_GeomFromText', `POLYGON(${latlon[0]} ${latlon[1]})`);
}

// function multipolygon(polygons) {
//     var sequelize = this.sequelize;
//     return sequelize.fn('ST_GeomFromText', `MULTIPOLYGON(${latlon[0]} ${latlon[1]})`);
// }
...
function getModel(sequelize) {
    ...
    if (!project) {
        project = sequelize.define('project', {
            ...
            latlon: Sequelize.GEOMETRY('POLYGON'),
